...
const THEME_TOKENS: Record<MiniAppTheme, {
  bg: string; headerBg: string; headerBorder: string; text: string; hint: string; fieldBg: string; dark: boolean; pattern: 'dots' | 'grid' | 'none'
}> = {
  aurora: { bg: '#ffffff', headerBg: '#ffffff', headerBorder: '#e7e5ea', text: '#1a1625', hint: '#8b8594', fieldBg: '#f4f2f8', dark: false, pattern: 'dots' },
  neon: { bg: '#0a0e1a', headerBg: '#0a0e1a', headerBorder: '#151d33', text: '#e8f2ff', hint: '#5f7690', fieldBg: '#111a2e', dark: true, pattern: 'grid' },
  mono: { bg: '#0a0a0a', headerBg: '#0a0a0a', headerBorder: '#1c1c1c', text: '#f4f4f5', hint: '#71717a', fieldBg: '#161616', dark: true, pattern: 'none' },
  glass: { bg: '#f0f9ff', headerBg: '#f7fcff', headerBorder: '#dbeafe', text: '#0c4a6e', hint: '#64748b', fieldBg: '#e6f4fd', dark: false, pattern: 'dots' },
  sunset: { bg: '#fff7ed', headerBg: '#fff7ed', headerBorder: '#fed7aa', text: '#431407', hint: '#a8825f', fieldBg: '#ffedd5', dark: false, pattern: 'none' },
}
...
export function StoreClient({ botId }: { botId: string }) {
  ...
  useEffect(() => {
    ...
    const onBack = () => setScreen(screen === 'checkout' ? 'detail' : 'store')
    tg.BackButton.show()
  ...
  if (loadError) {
    return (
      <Centered><p style={{ color: '#71717a', fontSize: 14 }}>{loadError}</p></Centered>
    )
  }
  if (!data) {
  ...
  return (
    <div style={{
      ...
      {screen === 'detail' && selected && (
        <DetailScreen product={selected} theme={theme} accent={accent} accent2={accent2} error={checkoutError} creating={creatingPix} onBuy={createCheckout} onBack={() => setScreen('store')} />
      )}
...
function StoreScreen({ config, theme, accent, accent2, categories, activeCategory, setActiveCategory, products, onSelect }: {
  ...
  theme: typeof THEME_TOKENS[MiniAppTheme]
  ...
  return (
    <div>
      <div style={{
        padding: '36px 20px 26px', textAlign: 'center', position: 'relative', overflow: 'hidden',
        background: `linear-gradient(135deg, ${accent}, ${accent2} 60%, ${theme.dark ? '#000' : '#00000022'})`,
      }}>
        <div style={{
          width: 56, height: 56, borderRadius: 16, background: 'rgba(255,255,255,0.95)',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26,
          margin: '0 auto 10px', boxShadow: '0 10px 22px rgba(0,0,0,0.25)',
        }}>
          {config.logo_emoji}
        </div>
        <p style={{ color: '#fff', fontSize: 18, fontWeight: 800, margin: 0 }}>{config.store_name}</p>
        <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: 12, margin: '3px 0 0' }}>{config.tagline}</p>
        {config.show_rating && (
          <span style={{
            display: 'inline-flex', marginTop: 10, background: 'rgba(255,255,255,0.22)',
          ...
          {categories.map((c) => (
            <button
              ...
              style={{
                ...
                padding: '7px 14px', borderRadius: 999, border: '1.5px solid transparent',
                background: activeCategory === c ? `${accent}22` : theme.fieldBg,
      ...
      <div style={{
        ...
        {products.map((p) => (
          <button
            ...
            onClick={() => onSelect(p)}
            style={{
              textAlign: 'left', cursor: 'pointer', border: 'none', padding: 0, borderRadius: 16, overflow: 'hidden',
          ...
          >
            <div style={{
              position: 'relative',
              height: config.layout === 'list' ? 60 : 80,
              width: config.layout === 'list' ? 60 : undefined,
              borderRadius: config.layout === 'list' ? 12 : undefined,
              margin: config.layout === 'list' ? 8 : undefined,
              flexShrink: 0,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26,
              background: `linear-gradient(135deg, ${accent}, ${accent2})`,
            }}>
              {p.miniapp_icon || '📦'}
              {p.miniapp_featured_label && (
                <span style={{
                  position: 'absolute', top: 6, left: 6, fontSize: 9, fontWeight: 800,
                  background: 'rgba(0,0,0,0.55)', color: '#fff', padding: '2px 6px', borderRadius: 5,
                }}>
                  {p.miniapp_featured_label}
                </span>
              )}
...
function DetailScreen({ product, theme, accent, accent2, error, creating, onBuy, onBack }: {
  product: Product; theme: typeof THEME_TOKENS[MiniAppTheme]; accent: string; accent2: string
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{
        height: 150, borderRadius: 18, marginBottom: 16, position: 'relative', overflow: 'hidden',
        background: `linear-gradient(135deg, ${accent}, ${accent2})`,
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff',
      }}>
        {product.miniapp_featured_label && (
          <span style={{ position: 'absolute', top: 10, left: 10, fontSize: 10, fontWeight: 800, background: 'rgba(0,0,0,0.4)', padding: '3px 9px', borderRadius: 6 }}>
            {product.miniapp_featured_label}
          </span>
        )}
        <span style={{ fontSize: 34, marginBottom: 6 }}>{product.miniapp_icon || '📦'}</span>
        <span style={{ fontSize: 15, fontWeight: 800, textAlign: 'center', padding: '0 20px' }}>{product.name}</span>
      </div>

      <p style={{ fontSize: 22, fontWeight: 800, color: theme.text, margin: '0 0 4px' }}>{formatBRL(product.price)}</p>
...
function CheckoutScreen({ product, theme, accent, checkout, status }: {
  product: Product; theme: typeof THEME_TOKENS[MiniAppTheme]; accent: string
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, background: theme.fieldBg, borderRadius: 14, padding: 11, marginBottom: 18 }}>
        <div style={{ width: 40, height: 40, borderRadius: 11, background: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, flexShrink: 0 }}>
          {product.miniapp_icon || '📦'}
        </div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0 14px' }}>
        <div style={{ width: 200, height: 200, borderRadius: 16, background: theme.fieldBg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 11, overflow: 'hidden' }}>
          {qrImgSrc ? (
